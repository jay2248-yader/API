...
exports.priceChecks = async (req, res, next) => {
    ...
    // FROM [CSV_STKWAREHOUSE2] WHERE PRODUCTCODE = '${productcode}'`;
        ...
        let sql = `SELECT  ST.CURRENCYCODE,ST.DOCDATE,ST.REMARK,ST.TAXTYPE,STS.PRODUCTCODE,STS.PRODUCTNAME,STS.UNITCODE,STS.SALEPRICE1 
        FROM CSSTDARPRICE ST INNER JOIN CSSTDARPRICESUB STS ON ST.DOCNO = STS.DOCNO 
        INNER JOIN CSBARCODE BAR ON BAR.PRODUCTCODE = STS.PRODUCTCODE
        WHERE ST.CLOSEFLAG = 0 AND STS.PRODUCTCODE = '${productcode}'  
        OR BAR.BARCODE = '${productcode}'
        ORDER BY ST.DOCDATE DESC`;
        
        const data = await conn.select(sql);
        let sql2 = `SELECT TOP 1000 
       W.[REBATE],
       W.[PRODUCTNAME],
       W.[PRODUCTCODE],
       W.[WAREHOUSE],
       ...
       W.[SEARCH_NAME],
       W.[SITECODE]
FROM [CSV_STKWAREHOUSE2] W
WHERE W.PRODUCTCODE = '${productcode}'
   OR EXISTS (SELECT 1 
              FROM CSBARCODE B 
              WHERE B.PRODUCTCODE = W.PRODUCTCODE 
...
exports.getProductByName = async (req, res, next) => {
    try {
        ...
        for (let i of products) {
            ps += `(c.NAMEEN LIKE N'%${i}%' OR c.CODE LIKE N'%${i}%' OR c.NAMETH LIKE N'%${i}%' OR b.BARCODE LIKE N'%${i}%') AND`;
        }
        let sqls = ` SELECT DISTINCT TOP 100 c.NAMEEN, c.NAMETH, c.CODE
    FROM [CSPRODUCT] c
    INNER JOIN [CSBARCODE] b ON b.PRODUCTCODE = c.CODE
    WHERE ${ps} c.CLOSEFLAG = 0`;
